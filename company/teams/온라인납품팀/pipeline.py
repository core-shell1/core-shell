...
def run(task: str = ""):
    ...
    # 콘텐츠 생성 전 구조화 (콘텐츠 품질 개선)
    if HAS_STRUCTURER:
        try:
            structure = structure_before_create(
                task=task,
                target_info={
                    "challenge": context.get("interview", "과제 미정"),
                    "reference_research": context.get("reference_research", "")[:500]
                },
                client=client
            )
            context = merge_structure_to_context(context, structure)
            save(output_dir, "_콘텐츠_구조.json", str(structure))
            print("✅ 콘텐츠 구조화 완료")
        except Exception as e:
            print(f"⚠️  콘텐츠 구조화 스킵: {e}")
    ...
    try:
        ...
        critique = auto_judge(critique, "온라인납품팀", client=client)
