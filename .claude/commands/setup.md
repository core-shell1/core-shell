...
`lian_company/.env` 파일이 없으면:
- `lian_company/.env.example`을 복사해서 `lian_company/.env` 생성
- `OWNER_NAME=` 항목을 Step 1에서 받은 이름으로 채워서 저장

`lian_company/.env` 파일이 이미 있으면 이 단계 스킵.
...
아래 명령어 실행:
```
cd lian_company && python -m venv venv && venv/Scripts/pip install -r requirements.txt
```
...
`lian_company/.env` 파일을 열어서 사용자에게 보여주고:
