...
> 보스가 "이거 해줘"라고 하면 이 문서를 보고 어떤 플로우를 태울지 스스로 판단해라.
> 마지막 업데이트: 2026-04-03
...
보스가 뭘 시키면 아래에서 해당하는 걸 골라서 실행해라.
...
보스가 직원 이름을 부르면 → 해당 직원만 단독으로 스폰해라.
...
예시: 보스가 "현우야 이 코드 아키텍처 봐줘" → `Agent(subagent_type="cto", prompt="...")`
